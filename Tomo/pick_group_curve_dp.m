function out = pick_group_curve_dp(ftan, freqs, times, distance_m, varargin)
...
%
% Output:
%   out struct: times_pick, vel_pick, slowness_pick, cost_map, path_cost, mask_valid, idx_path
...
if isempty(opt.vel_axis)
    ...
    vel_axis = opt.vel_axis(:).'; % ensure row vector
end

% ensure slowness row vector
slowness = 1 ./ vel_axis;
...
% small Gaussian smoothing along time axis
ftan_sm = ftan;
for i = 1:n_freq
...
if opt.weight_peak_boost
    for i = 1:n_freq
        ...
        if ~isempty(locs)
            peak_boost(i, locs) = 2; % simple boost factor
        end
    end
...
% --- EDGE REPEL: down-weight energy within K m/s of band edges
K = 100;          % edge thickness (m/s) to discourage hugging (tune 50–200)
alpha = 0.5;      % strength of down-weighting near edge (0=no effect, 1=strong)
p = 2;            % shape exponent (steeper near the edge)

edge_weight = ones(n_freq, n_time);
for i = 1:n_freq
    vmin = opt.vmin(min(i, numel(opt.vmin)));
    vmax = opt.vmax(min(i, numel(opt.vmax)));

    % distance to nearest edge (inside band only)
    dv_low  = max(0, vel_axis - vmin);
    dv_high = max(0, vmax - vel_axis);
    d_edge  = min(dv_low, dv_high); % zero exactly on the edge

    % only apply inside the band and within K of any edge
    inside = (vel_axis >= vmin) & (vel_axis <= vmax);
    near   = inside & (d_edge <= K);

    % weight shape: 1 - alpha*(1 - (d_edge/K))^p  in [1-alpha, 1]
    w = 1 - alpha*(1 - (d_edge./K)).^p;
    edge_weight(i, near) = w(near);
end

% apply to normalized energy before turning into cost
...
for i = 2:n_freq
    ...
    if ~any(valid_prev)
        cum_cost(i,:) = local_cost(i,:); % propagate local cost
        continue;
    end
    ...
    for r = 1:n_time
        ...
        sl_r = slowness(r);
        sl_prev_vec = slowness; % row vector
        sq = (sl_r - sl_prev_vec).^2;
        trans = opt.smoothness_lambda * sq;
        
        if ~isempty(opt.max_jump_m_s)
            v_r = vel_axis(r);
            bad = abs(vel_axis - v_r) > opt.max_jump_m_s;
            trans(bad) = big;
        end
...
if ~any(valid_end)
    idx_path = -ones(1,n_freq,'int32');
    times_pick = nan(1,n_freq);
    vel_pick = nan(1,n_freq);
    sl_pick = nan(1,n_freq);
else
    [~, end_idx] = min(last_col);
    path = zeros(1,n_freq,'int32'); path(n_freq) = int32(end_idx);
    for i = n_freq:-1:2
        prev = backptr(i, path(i));
        if prev < 1, path(1:i-1) = -1; break; end
        path(i-1) = prev;
    end
    
    times_pick = nan(1,n_freq); vel_pick = nan(1,n_freq); sl_pick = nan(1,n_freq);
    for i = 1:n_freq
        idx = path(i);
        if idx >= 1
...
out.path_cost = cum_cost(n_freq, path(end));
