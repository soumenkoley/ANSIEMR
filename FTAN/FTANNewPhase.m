function [fExt,distOut,SAIFFTAbs,tIFFT,symmCC,sc,figTT] = ...
    FTANNewPhase(inputCC,lagV,minAlpha,maxAlpha,fSamp,fExt,distIn,plotSet,caseNo,picNo,beta,tGrp)
...
for fNo = 1:1:length(fExt)
    ...
    phaseSA(:,fNo) = unwrap(angle(ifft(SAFilt)));
    if(fExt(fNo,1)>=1.6)
        if(~isnan(tGrp(fNo,1)))
            tInd = find(tIFFT>=tGrp(fNo,1),1,'first');
            phIn(fPhCount,1) = phaseSA(tInd,fNo);
            su = tGrp(fNo,1)/distIn;
            sc(fPhCount,1) = fExt(fNo,1);
            sc(fPhCount,2) = su + 1/(distOut*2*pi*fExt(fNo,1))*(phIn(fPhCount,1)-pi/4);
            fPhCount = fPhCount + 1;
        else
            sc(fPhCount,1) = fExt(fNo,1);
            sc(fPhCount,2) = NaN;
