...
for dayNo = 1:1:length(dayNumYear)
    ...
    foldPathA = ['Day',num2str(dayNumYear(dayNo))];
            ...
            if(exist(fullPathA))
                ...
                for endStnNo = 1:1:length(endStnList)
                    ...
                    if(~isempty(stnInd))
                        ...
                        ccHourly(:,ccN) = ccStore(:,stnInd);
                        ccHourly(:,ccN) = ccHourly(:,ccN)./max(abs(ccHourly(:,ccN)));
                        ccN = ccN+1;
...
for i = 1:1:lList
    for j = (i+1):1:(lList+1)
        ...
        stnStore(ccCount,1) = allStn(stnLocUse(i,3),1);
        stnStore(ccCount,2) = allStn(stnLocUse(j,3),1);
        ccCount = ccCount + 1;
    end
end
...
stnStore = stnStore((~isnan(ccStoreFinal(1,:))),:);
...
for i = 1:1:length(ccStoreFinal(1,:))
    ccStoreFilt(:,i) = filtfilt(HdBp1_5to3Hz.Numerator,1,ccStoreFinal(:,i));
end
...
distBin = min(rayAttribute(:,5)):10:max(rayAttribute(:,5));
...
figure(31);
imagesc((1:(24*15))/24,tArray,ccHourly(:,1:(24*15)));

figure(32);
plotseis(ccHourly(:,100:200),tArray,(100:200)/24);